...
```
runtime app 框架探测（per-app cache，几乎零成本）:

  Catalyst:
    bundle Info.plist 含 UIDeviceFamily / LSRequiresIPhoneOS
    或 main executable 路径 layout 是 iOS 风格

  Electron:
    .app/Contents/Frameworks/Electron Framework.framework/ 存在
...
这俩判定**靠 app 打包格式判定，跟版本无关，不需要人工维护**。苹果固定 Catalyst 是那 8-10 个 app 全家桶，Electron app 全部遵循统一打包规范。app 启动后 bundle 路径固定，**整个 app 生命周期只判定一次，缓存进 dict 即可**。
...
collectAll():
    app = frontmost app
    framework = detectFramework(app)       // cached, 0 IPC after first call

    // 显式覆盖优先：人工小列表
    if app.bundleID in AX_FORCE_BLACKLIST:
        run OmniParser, return                // e.g. 某 AppKit app 但 AX 烂
    if app.bundleID in AX_FORCE_WHITELIST:
        run AX walk, return                    // e.g. VS Code 这种 Electron 但 AX 不错

    switch framework:
        case .catalyst, .electron:
            run OmniParser, return            // 框架级判定，省一次 AX 扫描
        case .appkit, .swiftui, .unknown:
...
| 列表 | 内容 | 期望规模 |
| --- | --- | --- |
| `AX_FORCE_BLACKLIST` | 框架探测说 AppKit 但实际 AX 烂的（如某些用大量自渲染的老 app） | < 10 |
| `AX_FORCE_WHITELIST` | 框架探测说 Electron 但实际 AX 不错的 | VS Code、可能 Cursor、可能 Discord 较新版本——目前估 < 5 |

两个列表加起来 < 20 条。**框架探测覆盖了 90%+ 决策，列表只是 long-tail 修正**。
...
最初 spec 这一节按"count 阈值是主信号"写，按这个 framing 写了好几段。**讨论后翻盘**——`AX 干净的 app 不多 + 框架决定 AX 质量 + 框架可廉价探测` 三条加起来，"框架探测优先 + count 当安全网"是更对的 framing。这一节是改后版本，留这段史是为了不让未来读者重新走一遍这个推理。
