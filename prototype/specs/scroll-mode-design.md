...
- 按键进入滚动模式，**d/u 下/上滚动**焦点 app（垂直，wheel1）
- **`b`/`f` 左/右滚动**（水平，wheel2）—— Finder 列视图、宽表格、Notion DB、Figma 画布、日历周视图等场景
- 按住 d/u/b/f 连续滚，Shift+ 任一加速
- 多个可滚区域时，能选要滚哪个
...
### 2.4 浏览器真网页：Caps Lock+d 禁用，d/u 免模式直接滚

**例外**：当前台是浏览器、当前 tab 是注入了 content script 的真网页时，**Caps Lock+d 不进 SCROLL（直接吞掉无效果）**——因为这类页面 d/u/gg/G 已经能**免模式滚动**（Vimium 式），再进 SCROLL 没意义。判定靠扩展推来的 `scroll_gate` live 标志（`VimSession.browserHandlesScroll()`，前台 bundle 映射的 browser key == gate live 的 browser，且连接在线）。chrome:// / 网上应用店 / PDF 等无 content script 的页面仍正常进 SCROLL（兜底）。

免模式滚动本身**不属于 SCROLL 模式**：它由扩展 content script 检测按键、native 用一个常驻 `ScrollController`（不 enter/不 warp/不 overlay）在光标处发真滚轮实现。完整设计见 [`browser-support-design.md`](browser-support-design.md) §4.11，键位见 [`modes.md`](modes.md) §5.1。
