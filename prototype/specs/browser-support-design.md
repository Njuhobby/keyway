# Browser Support — Chrome / Firefox / Safari 扩展 + Native Messaging
...
Chrome 之外的第一个浏览器。结论：**便宜**——JS（`background.js` / `content_script.js` / `detector.js`）、`mouseless-bridge` stdio 二进制、native 端协议**全部复用**,只差 manifest 形态、native host 注册方式、几处浏览器识别。

实际改动:

- **manifest 分叉**:Chrome MV3 的 background 只认 `service_worker`,Firefox MV3 只认 `background.scripts`(event page),两者互斥 → 不可能一份 manifest 通吃。保留 `manifest.json`(Chrome)+ 新增 `manifest.firefox.json`(event page + `browser_specific_settings.gecko.id = mouseless@local` + `strict_min_version`)。`build-firefox.sh` 把共享 JS + firefox manifest 组装进 `dist-firefox/`(gitignored),Firefox `about:debugging` → Load Temporary Add-on 加载。**JS 不分叉**。
- **`chrome.*` → `browser.*` 兜底**:`background.js` 里大量 `await chrome.tabs.query(...)` 等。Firefox 的 `browser.*` 保证返回 promise,而 `chrome.*` 别名在部分版本是 callback 式。所以两个文件都用 `const chrome = globalThis.browser ?? globalThis.chrome;`——Chrome 上 `browser` undefined 即 no-op,Firefox 上走 promise 版。`detector.js` 纯 DOM、不碰扩展 API,无需改。
  - **坑(已修)**:这个 `const chrome` 在 `content_script.js` 里没事(它本来就在 IIFE 里,函数作用域允许遮蔽全局),但在 `background.js` 放**顶层**会炸——**Chrome MV3 的 service worker 把 `chrome` 当成顶层词法绑定**,顶层再 `const chrome` 直接 `SyntaxError: Identifier 'chrome' has already been declared` → **SW 注册失败、整个扩展后台挂掉**(Firefox 把 `chrome` 当 globalThis 属性,顶层 const 只是遮蔽,不报错,所以只在 Chrome 上爆)。修法:把整个 `background.js` **包进一个 IIFE**,`const chrome` 变函数作用域即可,调用点一个不用动;IIFE 同步执行,MV3 要求的"同步注册监听器"仍满足。
- **native host 注册**:Firefox 用 Mozilla 目录(`~/Library/Application Support/Mozilla/NativeMessagingHosts/`)、按 add-on id 的 `allowed_extensions`(而非 Chrome 的 `allowed_origins: chrome-extension://…`)。新增 `install_dev_host_firefox.sh`,gecko id 固定故无需传参。**bridge 二进制 + stdio 协议 + Unix socket 不变**。
- **浏览器识别**:扩展 `detectBrowser()` 加 `Firefox/` 分支返回 `"firefox"`;native `AppRegistry.browserBundleIDs` 与 `BridgeServer.browserKeyForBundleID` 加 `org.mozilla.firefox`(+ developeredition / nightly)。这样 hint 路由、Caps Lock+d 门控、`scroll_gate`、免模式滚动(§4.11)在 Firefox 上**全部自动生效**——新功能零额外成本。
