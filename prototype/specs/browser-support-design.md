...
|---|---|
| P0 install PoC | ✅ |
| P1 communication skeleton (BridgeServer / keyway-bridge CLI / extension long-lived connection) | ✅ |
...
Keyway's product thesis is *"press Caps Lock, one mental model across all apps"*—sticky / shift acceleration / hjkl cursor movement / drag / future new modes… all inside one unified interaction model. The moment we say "for the browser, please use Vimium", that thesis immediately collapses:

- **Sticky is unavailable in the browser**—Vimium is a one-shot hint-jump-press-exit flow
- **shift-accelerated scrolling / hjkl cursor drag / DRAG**—mouseEvents dispatched by browser JS aren't real mouse events, so they can't do this. Keyway uses OS-level `CGEvent` to actually "drag a Figma element / a Google Maps canvas"
...
```
            ...
            │  Chrome / Safari                │
            │  ┌───────────────────────────┐  │
            │  │ Keyway extension       │  │   JS, runs in the browser sandbox
            │  │  - background.js (SW)     │  │
            ...
            │ (keyway-bridge)              │   spawned by the browser,
            │                                 │   lifecycle owned by the browser
            └───────────┬─────────────────────┘
                        │ Unix domain socket
                        │ ~/Library/Application Support/Keyway/bridge.sock
            ┌───────────▼─────────────────────┐
            │ Keyway main process          │   existing Swift app
...
| **Keyway main process** | Detects that the frontmost is Chrome/Safari → goes through BrowserProvider instead of OP; renders the received hint list with the existing `HintOverlay`; after commit tells the extension which ID to click |
...
// extension → Keyway (DOM changed, proactively invalidate the current hint set)
{ "type": "invalidate", "tab_id": 12, "reason": "scroll" }
```

**Coordinate system**: the rects returned by the extension are already in **macOS screen global coordinates** (top-left origin), computed via `window.screenX/Y + window.devicePixelRatio + el.getBoundingClientRect()`. The Keyway side receives them without any further transform and feeds them straight to `HintOverlay`.

**Two click-commit paths**:

- Default `activate_at_cursor` — Keyway has already warped the cursor to the hint center and synthesized a `CGEvent` click. **Using a real mouse event has big upsides**: browsers, `<canvas>`, Flash-likes, and complex SPAs all respond correctly.
...
- Hint label rendering (it injects `<div>`s into the page; we let Keyway's `HintOverlay` draw)
- The modal key state machine (Keyway has its own mode system)
...
2. Don't connect to Keyway; the goal is only to verify the step "get clickable elements + coordinates inside the browser" works

**Acceptance**: N `{rect, text}` JSON entries printed in the console.

### P1 — Native Messaging Host + protocol stub (1-2 days) ✅

1. Write the `keyway-bridge` Swift CLI (~200 LOC): read length-prefixed JSON from stdin + write length-prefixed JSON to stdout
2. The Keyway main process opens a Unix socket listener, and bridge echoes messages back and forth with it
3. Register the host manifest at `~/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.keyway.bridge.json`
4. The extension background script does `chrome.runtime.connectNative('com.keyway.bridge')`, sends a ping, and the main process echoes it back

**Acceptance**: pressing a button in the extension produces a message visible in the Keyway main-process log, and the reply travels back to the extension and shows in the console.
...
- `keyway_hints_request {id, origin}` → parent asks child to collect
- `keyway_hints_response {id, hints}` → child returns (hint list in screen coordinates)
- `keyway_page_changed_inner` → child bubbles up to parent "I have new clickable elements here"
- 250ms timeout fallback (sandboxed / chrome:// iframes don't respond)

### P3 — Keyway-side BrowserProvider + Overlay rendering (1-2 days) ✅

Implementation points:

- New file `Sources/Keyway/BrowserProvider.swift`
...
- `BridgeServer.sendToActive` writes to the socket → bridge forwards to stdio → extension SW → content_script → detector → flows back in reverse; the Keyway side waits async with `awaitResponse(ofType:"hints", timeout:0.4)`
...
The original plan didn't account for multiple extension clients being connected at the same moment. The actual scenario is common: a user has Chrome Profile A + Profile B open at once (each profile is an independent extension install = independent SW = independent bridge process = independent socket connection to Keyway).

Getting it wrong is ugly: the user presses Caps Lock in Profile A, Keyway sends list_hints to the most-recently-connected fd (possibly Profile B), Profile B returns the hints of its own active tab, and they get drawn over Profile A.

**Fix**:

- On the extension side, `chrome.windows.onFocusChanged` listens → the SW sends `{type: "i_am_active"}` to Keyway. On SW startup it also proactively probes once with `chrome.windows.getLastFocused` (startup-race fallback).
...
**Fix**: extension-side `chrome.tabs.onActivated` listener; filter "is this a tab switch within this profile's current focused window"; send `{type: "tab_changed"}` to Keyway. On the Keyway side the handler routes `tab_changed` through the same path as `page_changed` (reuses `handlePageChanged()`, UX behavior identical).
...
**Fix is split across extension side + Keyway side**:
...
- iframe coordination: a **mirror** of the hints path—`keyway_text_request` / `keyway_text_response` (id, query, origin), 250ms timeout, off-viewport iframes culled the same way.
...
After an app switch the cursor warps by default to the midpoint of the focused window's title bar. Changing it to "if the focused window has an input, land inside that input" is a detail that makes Keyway smoother to daily-drive.
...
- **entering a mode auto-disables it**: after entering any Keyway mode the native tap swallows d/u before the page receives the key → the content script doesn't trigger → modeless scrolling stops. Zero coordination.
...
```jsonc
// extension → Keyway: whether the currently focused tab has a content script injected (gates Caps Lock+d)
...
// extension → Keyway: modeless scroll command (content script detected d/u/gg/G)
...
// Keyway → extension (internal to bg): content-script liveness probe
{ "type": "keyway_cs_alive" }   // content script replies { type:"cs_alive", alive:true }
```
...
The first browser beyond Chrome. Conclusion: **cheap**—the JS (`background.js` / `content_script.js` / `detector.js`), the `keyway-bridge` stdio binary, and the native-side protocol are **all reused**, differing only in manifest shape, native host registration, and a few browser-identification spots.

Actual changes:

- **manifest fork**: Chrome MV3's background only accepts `service_worker`, Firefox MV3 only accepts `background.scripts` (event page), and the two are mutually exclusive → no single manifest can serve both. Keep `manifest.json` (Chrome) + add `manifest.firefox.json` (event page + `browser_specific_settings.gecko.id = keyway@local` + `strict_min_version`). `build-firefox.sh` assembles the shared JS + the firefox manifest into `dist-firefox/` (gitignored), and Firefox `about:debugging` → Load Temporary Add-on loads it. **The JS doesn't fork**.
...
Once fired it reuses `notifyPageChangedThrottled()` (shares the 500ms throttle with the childList path), and on the Keyway side it goes through the ready-made `handlePageChanged` → refreshInPlace.
...
|---|---|
| **Cross-origin iframe** | postMessage chain protocol: each frame runs the detector itself, and after the child frame receives the parent frame's `keyway_hints_request` + parent-computed `viewportOriginInScreen` it recursively queries its own iframes → merges and returns. All hints are already in screen coordinates, so the top frame does no further coordinate transform. ✅ implemented |
...
- The user didn't install the extension → no web hints on Chrome. This requires **the extension to be part of the product, not optional**. After P5 store submission, what the user gets is a single installer bundling the extension + Keyway
- chrome:// / Web Store pages have no web hints. **Reasonable**—few people need Keyway's help on these pages anyway
...
- **Multi-tab hints** —— v1 only hints the current active tab; in the future it could extend to all tabs (so that when Keyway switches tabs for you it lands directly)
