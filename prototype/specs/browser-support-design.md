...
- **`chrome.*` → `browser.*` 兜底**:`background.js` 里大量 `await chrome.tabs.query(...)` 等。Firefox 的 `browser.*` 保证返回 promise,而 `chrome.*` 别名在部分版本是 callback 式。两个文件顶部加 `const chrome = globalThis.browser ?? globalThis.chrome;`——Chrome 上 `browser` undefined 即 no-op,Firefox 上走 promise 版。`detector.js` 纯 DOM、不碰扩展 API,无需改。
