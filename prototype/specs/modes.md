...
| 键 | 行为 |
| --- | --- |
| `a s d f g e r u i o p w t n m c v` | 输入 hint 标签字母（17 个；**注意不含 h/j/k/l**） |
...
为什么 hint 池没有 h/j/k/l：它们是 hjkl 移光标键，裸按一定是"移动"，不能再当 hint 字母（否则按 j 有歧义）。除 h/j/k/l 外其它顺手的字母都可用，`HintMode.alphabet` 取 **17 个**：`a s d f g e r u i o p w t n m c v`。17² = 289 > `maxTargets`(200)，所以**任何一次扫描都不会出现 3 字母 label**（2 字母封顶），label 池越大单字母 label 也越多（commit 更快）。顺序按手感前置（左手 home row 在前，单字母 label 用 `prefix` 取前缀、最短 label 最快 commit）。`v` 在末尾——它原本是 bare-key DRAG 入口、不能当 hint label，DRAG 改用 `Caps Lock + v` chord 之后腾出来了。详见 §4.3 与 `hint-rendering.md`。
...
sticky 下靠**两套机制**覆盖（都只在 sticky；non-sticky TAP 是一次性的，跟随焦点没意义）：

两条路径分开处理，**关键区别是焦点窗口变没变**：

**机制 1 — 切 app（焦点窗口变了）：立即重扫 + 截图只含新 app。** sticky TAP 整段挂一个 NSWorkspace `didActivateApplication` observer（`startAppSwitchFollow`，`enter()` 起、`exit()`/teardown 止，callback 里 `guard sticky`）。切 app（Cmd+Tab 或单击别的 app，两者都触发同一个通知、无法也无需区分）→ **立即** `rehintSticky(isolateApp: true)` 重扫新前台 app，**不加延迟**：通知触发时新 app 已是前台，没什么可等的。
...
**键位用途速查**（TAP mode）：`h/j/k/l` = hjkl 移光标；hint 字母 = `a s d f g e r u i o p w t n m c v`（17 个，不含 hjkl）；`Enter` = 点击；数字 = Dock hint / SCROLL 切区域。
