...
```swift
enum Mode {
    case tap(HintMode)            // hint 点击 + hjkl 移光标 + Enter 点击 + (子状态：drag / search)
...
| 键 | 行为 |
| --- | --- |
| `a s d f g e r u i o p w t n m c` | 输入 hint 标签字母（16 个；**注意不含 h/j/k/l/v**） |
...
为什么 hint 池没有 h/j/k/l/v：h/j/k/l 是 hjkl 移光标键、v 是 bare-key 进 DRAG 子状态，裸按它们都有专门含义、不能再当 hint 字母（否则歧义）。除这五个外其它顺手字母都可用，`HintMode.alphabet` 取 **16 个**：`a s d f g e r u i o p w t n m c`。16² = 256 > `maxTargets`(200)，所以**任何一次扫描都不会出现 3 字母 label**（2 字母封顶）。顺序按手感前置（左手 home row 在前，单字母 label 用 `prefix` 取前缀、最短 label 最快 commit）。详见 §4.3 与 `hint-rendering.md`。
...
**键位用途速查**（TAP mode）：`h/j/k/l` = hjkl 移光标；hint 字母 = `a s d f g e r u i o p w t n m c`（16 个，不含 hjkl/v）；`v` = bare 进 DRAG 子状态；`/` = bare 进 search 子状态；`Enter` = 点击；数字 = Dock hint / SCROLL 切区域。
