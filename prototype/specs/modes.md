...
```swift
enum Mode {
    case tap(HintMode)            // hint 点击 + hjkl 移光标 + Enter 点击
...
- **Mode** 描述 "用户当前在干什么"。当前两个：`.tap`（hint 点击，也含 hjkl 移光标 + Enter 点击）、`.scroll`（键盘滚动）。移光标键 **hjkl 在两个模式统一**（消除模式间认知切换）。
...
[OFF] ──Caps Lock 松手(无 chord)──> [TAP] ──Caps Lock 松手──> [TAP sticky]
   ...
   [TAP] Caps Lock按住+d ───┤        ├─ Enter          → 光标位置左键单击
                            ▼        ├─ Shift+; (:)    → 命令面板
...
| 键 | 行为 |
| --- | --- |
| `a s d f g e r u i o p w t n m c` | 输入 hint 标签字母（16 个；**注意不含 h/j/k/l**） |
...
| `Enter` (bare) | 当前光标位置 **左键单击**（配 hjkl：移到位 → Enter 点击） |
| `Shift + Enter` | 当前光标位置 **双击** |
| `Option + Enter` | 当前光标位置 **右键** |
| `Backspace` | 撤销上一个输入的 hint 字符（typed 空时无操作，不退出） |
| `Shift + ;` (= `:`) | 打开命令面板 |
| `Cmd / Ctrl + 任意键` | **放行**，不消费（保系统快捷键） |

为什么 hint 池没有 h/j/k/l：它们是 hjkl 移光标键，裸按一定是"移动"，不能再当 hint 字母（否则按 j 有歧义）。除 h/j/k/l 外其它顺手的字母都可用，`HintMode.alphabet` 取 **16 个**：`a s d f g e r u i o p w t n m c`。16² = 256 > `maxTargets`(200)，所以**任何一次扫描都不会出现 3 字母 label**（2 字母封顶），label 池越大单字母 label 也越多（commit 更快）。顺序按手感前置（左手 home row 在前，单字母 label 用 `prefix` 取前缀、最短 label 最快 commit）。详见 §4.3 与 `hint-rendering.md`。
...
**问题：画面变化（点击导致 或 切 app）让 hint 失效。** hint overlay 是按某个 app 的布局画的（`.statusBar` 级、跨所有 space、`orderFrontRegardless`，**不跟随焦点**）。两件事会让它失效：

1. **切 app**（Cmd+Tab，或点击打开别的 app）—— overlay 还停在旧 app 的坐标上盖着新 app。
2. **同 app 内容变化**（列表选中 → 详情面板重载、展开 disclosure、原地导航、弹 popover、开新窗口）—— 合成 click 是异步的，`rehintSticky()` 立即跑时 walk 的是**点击前**的树，重扫出来还是旧画面（用户实测踩到）。

sticky 下靠**三套机制**覆盖（都只在 sticky；non-sticky TAP 是一次性的，跟随焦点没意义）：

**机制 1 — 切 app：always-on app-switch follow。** sticky TAP 整段挂一个 NSWorkspace `didActivateApplication` observer（`startAppSwitchFollow`，`enter()` 起、`exit()`/teardown 止，callback 里 `guard sticky`）。切 app → `scheduleStickyRehint()` 重扫**新前台 app**（按新 app 的路由决定立即还是延后）。app 激活是对**所有 app 都可靠**的信号（NSWorkspace，AX 无关），所以这条全程开着。

**机制 2 — 同 app 内容变化（AX 白名单 app，post-commit）：`PostCommitWatcher`。** commit 后立即 `rehintSticky()` 给同步更新即时反馈；同时 `PostCommitWatcher`（notification 驱动，非轮询）在 app 元素上监听同 app 的窗口类（`kAXFocusedWindowChanged` / `kAXMainWindowChanged` / `kAXWindowCreated`）+ 内容类（`kAXValueChanged` / `kAXLayoutChanged` / `kAXSelectedRows`/`ChildrenChanged` / `kAXRowCountChanged` / `kAXCreated` / `kAXUIElementDestroyed` / `kAXTitleChanged`）。一串通知（旧面板 destroyed → 新元素 created → layoutChanged …）用 **debounce**（100ms，每来一条重置）合并成**一次**重扫；`maxLatencyMs`（400ms）兜底防持续刷新的 app；硬超时 700ms 内可多次 fire（多阶段加载），每次都被 `rehintGeneration` 守护，最新胜出。**只听 700ms + 仅 post-commit**：内容通知很吵（计时器一直发 `valueChanged`），常开会狂重扫。

**机制 3 — 同窗口内容变化（OmniParser app）：重扫延后 ~100ms。** webview / Electron / Clash Verge 等 **不发 AX 变化通知**（正因为 AX 烂才走 OP），机制 2 对它们**永不 fire**——实测 Clash Verge 点击后 0 条通知。所以 `scheduleStickyRehint` 按路由分流：AX app 立即重扫（机制 2 兜异步），**OP app 延后 ~100ms** 再 `rehintSticky()`，让点击生效、画面稳定后再截图 + 重跑 OP（否则截的是点击前那帧）。延后期间再点会取消上一个 pending（最新胜出），退出 / 切 mode 也取消。延时可调。**没用截图轮询**：一次截图 ~50ms ≈ 一次 OP 推理，轮询比直接多重扫 2~3 次还贵，不划算。

> 历史：最早只有 focus-only watcher（只监听焦点窗口变化），只覆盖第 1 类，同窗口内容变化（A 类 app）和 OP app 全 stale，是这个 bug 的根。

### 4.3 `Enter` —— 光标位置点击 + hjkl 移光标

`Enter` 与 hjkl 配套，构成 TAP 内的"键盘鼠标"：**hjkl 移光标对准，Enter 点击**。Enter 比原来的 `x` 更直觉（确认/点击）。
...
- **`Enter`** = 在**当前鼠标光标位置**合成点击。修饰键选类型，跟 hint commit 一致：bare = 左键单击、`Shift+Enter` = 双击、`Option+Enter` = 右键（共用 `clickKind(from:)`）。落点 = 光标现在在哪。Enter 在 palette 里是"执行命令"，但 palette 拦截在前；hint 标签是字母/数字，Enter 不冲突；mode 激活时 Enter 被消费、不下发 app。
...
**历史**：`x` 曾是"dismiss 所有打开的菜单 + 重扫"手势（Dock 菜单 AXCancel + 焦点切换关浮层）。撞 OmniParser 异步化和"键盘鼠标"设计后改成现在的纯光标点击——更简单、跟 hjkl 移动闭环。旧的 `findOpenDockMenu` / `AXCancel` / `AXWait.appActivated` 那套已从代码移除。**关菜单能力丢失**（光标在控件上点会误触发），可接受。
...
| `Shift / Option + Enter` | 当前光标位置 双击 / 右键 |
| `数字键 1-9` | 切换选中的滚动区域 |
| Caps Lock 单击 | 切回 TAP（见 §2.1） |
| `Esc` | deactivate 回 OFF |

**移光标用 hjkl，与 TAP 完全统一**：早期 SCROLL 用 SDFE、TAP 用 IJKL，两套移动键逼用户在模式间切换肌肉记忆——是真实的认知负担。现统一成 vim hjkl（`VimSession.moveDirection(for:)` 单一映射，两模式共用），"移光标"在哪都一样，只有滚动/点击因模式而异。滚动因此从 `j/k` 改到 **`d`(下)/`u`(上)**——`j/k` 让给移光标，`d` 也正好对上进入 SCROLL 的 chord 键。`Enter` 与 hjkl 配套（移→点闭环），复用 `MouseMover` / `MouseSynth`（与 TAP 同一套）。
...
**键位用途速查**（TAP mode）：`h/j/k/l` = hjkl 移光标；hint 字母 = `a s d f g e r u i o p w t n m c`（16 个，不含 hjkl）；`Enter` = 点击；数字 = Dock hint / SCROLL 切区域。
