...
> **app-switch 那条路（机制 1 / `reapplyOnCurrentFrontmost`）目前还是固定 100ms/500ms 盲延迟**，同样有"激活后内容渲染慢就扫到半截"的隐患（跨 Space 那种靠 `activeSpaceDidChange` 在动画结束时兜了底，同 Space 那条是纯猜）。这套小图 watch 以后可原样搬过去（激活后启动 watch、settle 再扫），把它也变成"等画面稳定再扫"。今天先只改 commit 这条。
