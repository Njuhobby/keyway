...
| 键 | 行为 |
| --- | --- |
| `a s d f g e r u i` | 输入 hint 标签字母（9 个；**注意不含 h/j/k/l**） |
...
为什么 hint 池没有 h/j/k/l：它们是 hjkl 移光标键，裸按一定是"移动"，不能再当 hint 字母（否则按 j 有歧义）。所以 `HintMode.alphabet` 移除 h（j/k/l 原本就因旧 IJKL 不在），补 i 维持 9 键 → `a s d f g e r u i`。详见 §4.3。
...
| `tab` | 48 | 暂未用 |
| `space` | 49 | 暂未用 |
| `a..l` | 0,1,2,3,5,4,38,40,37 | home row 9 键（注意 g/h 顺序：g=5, h=4）；`h/j/k/l` = hjkl 移光标 |
| `i` | 34 | hint 字母补位（上排，非 home row） |
| `q..p` | 12,13,14,15,17,16,32,34,31,35 | 上排（`e=14 r=15 u=32 i=34` 现作 hint 字母补位） |
...
**键位用途速查**（TAP mode）：`h/j/k/l` = hjkl 移光标；hint 字母 = `a s d f g e r u i`（不含 hjkl）；`Enter` = 点击；数字 = Dock hint / SCROLL 切区域。
