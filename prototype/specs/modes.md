...
**机制 1 — 切 app（焦点窗口变了）：旧 overlay 立即隐藏 + 100ms 后重扫 + 截图只含新 app。** sticky TAP 整段挂一个 NSWorkspace `didActivateApplication` observer（`startAppSwitchFollow`，`enter()` 起、`exit()`/teardown 止，callback 里 `guard sticky`）。切 app（Cmd+Tab 或单击别的 app，两者都触发同一个通知、无法也无需区分）→ 立即 `deactivate()` 老 HintMode（藏掉旧坐标的 overlay）+ `scheduleStickyRehint(isolateApp: true)` 100ms 后重扫新前台 app。
