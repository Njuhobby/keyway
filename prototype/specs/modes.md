...
```swift
enum Mode {
    case tap(HintMode)            // hint 点击 + IJKL 移光标 + x 点击
...
- **Mode** 描述 "用户当前在干什么"。当前两个：`.tap`（hint 点击，也含 IJKL 移光标 + x 点击）、`.scroll`（键盘滚动）。
...
[OFF] ──Caps Lock 松手(无 chord)──> [TAP] ──Caps Lock 松手──> [TAP sticky]
   ...
   [TAP] Caps Lock按住+j/k ──┤        ├─ x              → 光标位置左键单击
                             ▼        ├─ Shift+; (:)    → 命令面板
...
| `Shift + 末位字符` | 双击 |
| `Option + 末位字符` | 右键点击 |
| `i / j / k / l` (bare) | **移光标** 上/左/下/右（IJKL 倒 T，按住连续） |
| `Shift + ijkl` | 加速移光标 |
| `x` (bare) | 当前光标位置 **左键单击一次** |
| `Shift + ;` (= `:`) | 打开命令面板 |
...
### 4.3 `x` —— 光标位置点击 + IJKL 移光标

`x` 与 IJKL 配套，构成 TAP 内的"键盘鼠标"：**IJKL 移光标对准，x 点击**。
...
- **`x`** = 在**当前鼠标光标位置**合成一次左键单击。落点 = 光标现在在哪。
- **`i/j/k/l`** = 移光标（IJKL 倒 T：i 上、j 左、k 下、l 右），按住连续（60fps timer 合成 `.mouseMoved`，hover 状态会更新），Shift 加速。实现见 `MouseMover.swift`。
...
| `Shift + j/k` | 加速滚动 |
| `s/d/f/e` (bare) | 移光标 左/下/右/上（SDFE 倒 T，按住连续，Shift 加速） |
| `x` (bare) | 当前光标位置左键单击（留在 SCROLL） |
| `数字键 1-9` | 切换选中的滚动区域 |
| Caps Lock 单击 | 切回 TAP（见 §2.1） |
| `Esc` | deactivate 回 OFF |

**移光标为什么用 SDFE，且跟 TAP 的 IJKL 不一致**：scroll 里 `j/k` 已是滚动，腾不出 IJKL。SDFE（e 上 / s 左 / d 下 / f 右，倒 T 中心在 d）让**左手不离 home row**——s/d/f 正是无名指/中指/食指的 home 位，e 只需中指上抬，不像 WASD 要小指够 'a'。左手移光标、右手滚动可**双手并行**。TAP 用 IJKL 是因为它的 home row `a/s/d/f` 要留给 hint 字母，scroll 没有 hint 所以 home row 空出来给移动。`x` 与 SDFE 配套（移→点闭环），复用 `MouseMover` / `MouseSynth`（与 TAP 同一套）。
...
| `Shift` | 消费 —— hint 末位字符上 = 双击；IJKL 上 = 加速移动 | 高频动作配顺手修饰键 |
| `Option` | 消费 —— hint 末位字符上 = 右键 | hint click action 需要 |
