...
**hint label commit** 的点击类型由修饰键决定（bare `c` **不走这张表**——它有自己的 `cc` 双击,见 §4.4）:
...
`clickAction(for:)` 读这个手势状态,**仅 hint label commit 用**。bare `c`(TAP+SCROLL)已改用自己的 `cc` 双击(`handleBareCClick`,见 §4.4),不再读 `shiftDoubleArmed`。
...
- **`Shift+c` = 右键**(立即,不参与单/双判定)。
  - （旧方案 `c` 的双击走"Shift 双击 + c",已废,换成更直觉的 `cc`;hint label 的双击仍是 Shift 双击,见 §4.3.6。）
...
| `Shift` | 消费 —— hint 末位字符 / `c` = **右键**（按住）；Shift **双击按住** + label = **双击**（§4.3.6；`c` 的双击改用 `cc`，见 §4.4）；移光标键(hjkl) = 加速；hjkl 双击 = 跳整屏（否则 1/2） | Shift 潜意识 = "另一种点击" = 右键；双击让给 Shift 双击手势 |
