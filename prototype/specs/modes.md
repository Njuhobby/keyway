...
**机制 1 — 焦点窗口变了（切 app **或者** 同 app 内换窗）：旧 overlay 立即隐藏 + 100ms 后在新 frontmost 上重新 apply 当前模式 + 截图只含新 app。** 整个 active session（TAP / SCROLL / WINDOW / MOVE 都算）挂三条信号，全部汇到 `reapplyOnCurrentFrontmost`：

| 信号 | 触发场景 | 延迟 |
|---|---|---|
| `NSWorkspace.didActivateApplication` | Cmd+Tab 切 app / 单击别的 app | ~0ms（事件驱动） |
| `NSWorkspace.activeSpaceDidChange` | 跨 Space 滑动（Ctrl+方向键 / 三指滑），或 cross-Space 切 app 完成动画 | ~0ms（事件驱动） |
| 150ms `focusedWindowPollTimer` | **同 app 内** focused window 变了：Cmd+W 关窗 / Cmd+M 最小化 / Cmd+\` 切窗 / Cmd+N 开新窗 | 最坏 150ms |

三条信号 + 一个 handler 的扇入设计。handler 不关心是谁叫的，它就一件事：**读当前 frontmost app / window，在那个状态上重画**。同一逻辑事件可能多条线一起 fire（例：cross-Space Cmd+Tab 时 didActivateApplication 立刻发，300ms 后 activeSpaceDidChange 又发，poll 在中间也可能看到 window 变化）——`reapplyOnCurrentFrontmost` 每次进来都 `cancel` 上次 pending 的 reapply DispatchWorkItem 再重新 schedule，**setup 跑 N 次但实际 re-enter 只跑 1 次**（最后那次胜出）。同时进来时刷一下 poll 缓存的 `lastSeenFocusedWindow`，让晚到的信号看不到变化、不再触发。
...
| `Shift` | 消费 —— hint 末位字符 / Enter = 双击；移光标键(hjkl) = 加速；hjkl 双击 = 跳 1/2 屏（否则 1/4） | 高频动作配顺手修饰键 |
