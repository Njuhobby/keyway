...
> **`activeSpaceDidChange` 也直接订阅了**，用作两类信号：(1) 上面说的 cross-Space 加速；(2) 用户主动切 Space（Ctrl+方向键 / 三指滑），没有 app activation 但焦点 app 切了，也要在新 Space 重 apply。两类都走 `handleSpaceChanged` → `handleAppActivated`，pending 取消+重新调度的机制天然 dedup。
