...
echo "built: $OUT"
echo "  manifest ← manifest.firefox.json (gecko id: keyway@local)"
echo ""
echo "next:"
echo "  1. ./install_dev_host_firefox.sh   (register the native host for Firefox)"
echo "  2. Keyway main process running (./run.sh from prototype/)"
echo "  3. Firefox → about:debugging#/runtime/this-firefox"
echo "     → \"Load Temporary Add-on…\" → pick $OUT/manifest.json"
echo "  4. click the Keyway toolbar icon to trigger the first native ping"
