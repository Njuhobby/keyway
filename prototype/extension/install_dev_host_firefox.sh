#!/usr/bin/env bash
# Install the Firefox Native Messaging host manifest for the Mouseless
...
# The gecko ID is fixed in manifest.firefox.json (mouseless@local), so —
...
# pointing at this checkout's `mouseless-bridge` binary. The SAME bridge
# binary + stdio protocol as Chrome — only the host-manifest location and
# the allow-list key differ. Idempotent.

set -euo pipefail

GECKO_ID="mouseless@local"   # keep in sync with manifest.firefox.json
...
echo "  3. click the Mouseless toolbar icon to trigger the first ping"
