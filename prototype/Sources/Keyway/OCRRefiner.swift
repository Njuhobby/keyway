...
enum OCRRefiner {
    ...
    static func recognizeText(in image: CGImage) -> [VNRecognizedTextObservation] {
        ...
        do {
            ...
            Log.warn("[keyway] OCR recognizeText failed: \(error)")
            return []
        }
    ...
    static func refine(
        ...
        if !centerInInner {
            Log.debug("[keyway] OCR refiner: center-no-conflict, skip OCR — point=(\(Int(center.x)),\(Int(center.y)))")
            return center
        }
        ...
        guard let captured = await ScreenCapture.captureFocusedWindow() else {
            Log.debug("[keyway] OCR refiner: screen recapture nil — fallback to box center")
            return fallback
        }
        ...
        guard windowRect.intersects(boxScreenRect) else {
            Log.debug("[keyway] OCR refiner: box outside current window — fallback to box center")
            return fallback
        }
        ...
        guard let crop = image.cropping(to: clampedBoxInPx) else {
            Log.warn("[keyway] OCR refiner: crop failed (rect=\(clampedBoxInPx)) — fallback")
            return fallback
        }
        ...
        do {
            ...
            Log.warn("[keyway] OCR refiner: perform failed (\(error)) — fallback")
            return fallback
        }
        ...
        Log.debug("[keyway] OCR refiner: cap=\(ms(tStart, tCap))ms ocr=\(ms(tCap, tOCR))ms total=\(ms(tStart, tEnd))ms regions=\(textRegions.count) mode=\(mode) point=(\(Int(clickPoint.x)),\(Int(clickPoint.y)))")
        return clickPoint
    }
